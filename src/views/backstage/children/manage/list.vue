        ...
        <el-row>
          ...
          <el-col :span="11" :offset="2">
            <div class="cell">
              ...
              <el-form-item label="来源：" prop="mFrom">
              <el-input v-model="addObject.mFrom" placeholder="请输入内容" class="flew-input"></el-input>
              </el-form-item>
            </div>
          </el-col>
        </el-row>
        ...
        <el-row>
          ...
          <el-col :span="11" :offset="2">
            <div class="cell">
              <el-form-item label="来源：" prop="mFrom">
              <!--<span class="name">来源：</span>-->
              <el-input v-model="editObject.mFrom" placeholder="请输入内容" class="flew-input"></el-input>
              </el-form-item>
            </div>
          </el-col>
        </el-row>
  ...
  export default {
    ...
    methods: {
      ...
      getPage() {
        ...
        API.get('/management/findAll', params,{Authorization:storage.get('token')}).then((res) => {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if(this.num == 1) {
              ...
              params['mFrom'] = this.addObject.mFrom;
              params['systemId'] = storage.get('sysid');
              // console.log(params);
              API.post('/management/create', params,{Authorization:storage.get('token')}).then((res) => {
                // console.log(res.data);
                if (res.data.code == 200) {
                  this.addPop = false;
                  this.getPage();
      ...
      editOpen(id) {
        ...
        let params = {};
        params['id'] = id;
        API.get('/management/findById', params,{Authorization:storage.get('token')}).then((res) => {
          // console.log(res.data);
          if (res.data.code == 200) {
          ...
          if (valid) {
            if(this.EditName == this.editObject.title){
              ...
              if(this.num == 1) {
                ...
                params['mFrom'] = this.editObject.mFrom;
                params['systemId'] = storage.get('sysid');
                // console.log(params);
                API.post('/management/update', params,{Authorization:storage.get('token')}).then((res) => {
                  if (res.data.code == 200) {
                    this.editPop = false;
                    this.getPage();
                    this.$message({
              ...
              API.get('/management/findByTitle', tit, {Authorization: storage.get('token')}).then((res) => {
                if (res.data.code == 200) {
                  this.num ++;
                  if(this.num == 1) {
                    this.loadingBtn = true;
      ...
      del(id) {
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          params['id'] = id;
          params['systemId'] = storage.get('sysid');
          API.delete('/management/delete', params,{Authorization:storage.get('token')}).then((res) => {
            if (res.data.code == 200) {
              this.currentPage = 1;
              this.pageSize = 10;
              this.getPage();
      ...
      // 查看
      linkDetail(id) {
        this.$router.push({name: 'backstage.manage.detail', query: {id: id}});
      },
